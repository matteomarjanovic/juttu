...
`;function Y(u){if(!u.startsWith("at://"))return null;let e=u.slice(5).split("/");if(e.length!==3)return null;let[t,n,r]=e;return!t||!n||!r?null:{did:t,collection:n,rkey:r}}async function K(u){let e;if(u.startsWith("did:plc:")){let n=await fetch(`https://plc.directory/${u}`);if(!n.ok)throw new Error(`Failed to resolve DID: ${n.status}`);e=await n.json()}else if(u.startsWith("did:web:")){let n=u.split(":").slice(2).join(":"),r=await fetch(`https://${n}/.well-known/did.json`);if(!r.ok)throw new Error(`Failed to resolve did:web DID: ${r.status}`);e=await r.json()}else throw new Error(`Unsupported DID method: ${u}`);let t=e.service?.find(n=>n.type==="AtprotoPersonalDataServer");if(!t?.serviceEndpoint)throw new Error("No PDS endpoint found in DID document");return t.serviceEndpoint}async function Q(u,e){let t=`${u}/xrpc/com.atproto.repo.getRecord?repo=${encodeURIComponent(e.did)}&collection=${encodeURIComponent(e.collection)}&rkey=${encodeURIComponent(e.rkey)}`,n=await fetch(t);if(n.status>=400&&n.status<500)return null;if(!n.ok)throw new Error(`Failed to fetch document record: ${n.status}`);return(await n.json()).value}async function S(u){let e=`https://public.api.bsky.app/xrpc/app.bsky.feed.getPostThread?uri=${encodeURIComponent(u)}&depth=10&parentHeight=0`,t=await fetch(e);if(!t.ok)throw new Error(`Failed to fetch thread: ${t.status}`);let n=await t.json();if(!n.thread)throw new Error("Thread data missing from response");return n.thread}async function C(u){try{let e=await fetch(`${u}/auth/me`,{credentials:"include"});return e.ok?await e.json():null}catch{return null}}async function L(u){try{let e=await fetch(`https://public.api.bsky.app/xrpc/app.bsky.actor.getProfile?actor=${encodeURIComponent(u)}`);if(!e.ok)return{};let t=await e.json();return{avatar:t.avatar,displayName:t.displayName}}catch{return{}}}function I(u){let e=Math.floor((Date.now()-new Date(u).getTime())/1e3);return e<60?"just now":e<3600?`${Math.floor(e/60)}m`:e<86400?`${Math.floor(e/3600)}h`:e<2592e3?`${Math.floor(e/86400)}d`:e<31536e3?`${Math.floor(e/2592e3)}mo`:`${Math.floor(e/31536e3)}y`}function it(u){return u.startsWith("https://")||u.startsWith("http://")}function X(u,e){let t=document.createDocumentFragment(),n=new TextEncoder,r=new TextDecoder,o=n.encode(u);if(!e||e.length===0)return t.appendChild(document.createTextNode(u)),t;let a=[...e].sort((s,l)=>s.index.byteStart-l.index.byteStart),i=0;for(let s of a){let{byteStart:l,byteEnd:c}=s.index;l>i&&t.appendChild(document.createTextNode(r.decode(o.slice(i,l))));let d=r.decode(o.slice(l,c)),h=s.features[0];if(!h)t.appendChild(document.createTextNode(d));else if(h.$type==="app.bsky.richtext.facet#link")if(it(h.uri)){let p=document.createElement("a");p.className="juttu-link",p.href=h.uri,p.target="_blank",p.rel="noopener noreferrer",p.textContent=d,t.appendChild(p)}else t.appendChild(document.createTextNode(d));else if(h.$type==="app.bsky.richtext.facet#mention"){let p=document.createElement("a");p.className="juttu-mention",p.href=`https://bsky.app/profile/${h.did}`,p.target="_blank",p.rel="noopener noreferrer",p.textContent=d,t.appendChild(p)}else if(h.$type==="app.bsky.richtext.facet#tag"){let p=document.createElement("a");p.className="juttu-hashtag",p.href=`https://bsky.app/search?q=${encodeURIComponent("#"+h.tag)}`,p.target="_blank",p.rel="noopener noreferrer",p.textContent=d,t.appendChild(p)}else t.appendChild(document.createTextNode(d));i=c}return i<o.length&&t.appendChild(document.createTextNode(r.decode(o.slice(i)))),t}function tt(u){return(u.replies??[]).filter(e=>e.$type==="app.bsky.feed.defs#threadViewPost")}function z(u,e){return[...u].sort((t,n)=>e==="most-liked"?(n.post.likeCount??0)-(t.post.likeCount??0):e==="oldest"?new Date(t.post.indexedAt).getTime()-new Date(n.post.indexedAt).getTime():new Date(n.post.indexedAt).getTime()-new Date(t.post.indexedAt).getTime())}function E(u,e){let{post:t}=u;(t.viewer?.like||t.viewer?.repost)&&e.set(t.uri,{likeUri:t.viewer.like,repostUri:t.viewer.repost});for(let n of u.replies??[])n.$type==="app.bsky.feed.defs#threadViewPost"&&E(n,e)}function D(u,e){if(!u)return null;if(u.post.uri===e)return u.post;for(let t of u.replies??[])if(t.$type==="app.bsky.feed.defs#threadViewPost"){let n=D(t,e);if(n)return n}return null}var W=/@([a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+)/g,Z=/https?:\/\/[^\s\x00-\x1f<>"]+/g,G=/(^|[\s(])((?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}(?:\/[^\s\x00-\x1f<>"]*)?)/g,J=/#[^\s\x00-\x1f.,;!?\-#]{1,64}/g,H=/[.,;!?)\]]+$/;function at(u){let t=u.split("/")[0].split(".").at(-1)??"";return t.length>=2&&/^[a-z]+$/i.test(t)}function et(u){let e=[],t;for(Z.lastIndex=0;(t=Z.exec(u))!==null;){let i=t[0].replace(H,"");i&&e.push({start:t.index,end:t.index+i.length,type:"link"})}for(G.lastIndex=0;(t=G.exec(u))!==null;){let i=t[1]??"",s=t[2];if(!at(s))continue;let l=t.index+i.length,c=s.replace(H,"");c&&e.push({start:l,end:l+c.length,type:"link"})}for(J.lastIndex=0;(t=J.exec(u))!==null;){let i=t[0].replace(H,"");i&&e.push({start:t.index,end:t.index+i.length,type:"tag"})}for(W.lastIndex=0;(t=W.exec(u))!==null;){let i=t.index;if(i>0){let s=u[i-1];if(s!==" "&&s!=="	"&&s!==`
`&&s!=="(")continue}e.push({start:i,end:i+t[0].length,type:"mention"})}e.sort((i,s)=>i.start-s.start);let n=[],r=0;for(let i of e)i.start<r||(n.push(i),r=i.end);let o=[],a=0;for(let i of n)i.start>a&&o.push({text:u.slice(a,i.start),type:"plain"}),o.push({text:u.slice(i.start,i.end),type:i.type}),a=i.end;return a<u.length&&o.push({text:u.slice(a),type:"plain"}),o}var T=class{config;container;threadData=null;currentUser=null;rootPostUri=null;rootPostCid=null;sortOrder="newest";pagination={visibleTopLevel:10,visibleReplies:new Map};viewerState=new Map;openReplyFormUri=null;loginPopup=null;loginPollInterval=null;loginPollStartTime=0;mentionDebounceTimer=null;activeSuggestionsTextarea=null;documentAtUri=null;documentRecord=null;linkingStep="setup";linkingTitle="";linkingDescription="";userPosts=[];constructor(e,t){this.container=e,this.config=t,this.injectStyles(),this.init()}injectStyles(){if(document.getElementById("juttu-styles"))return;let e=document.createElement("style");e.id="juttu-styles",e.textContent=q,document.head.appendChild(e)}async init(){this.renderLoading();try{let e=document.querySelector('link[rel="site.standard.document"]');if(!e){this.renderError('Missing <link rel="site.standard.document"> tag on this page. Add it to enable comments.');return}let t=e.getAttribute("href");if(!t||!t.startsWith("at://")){this.renderError('Invalid <link rel="site.standard.document" href> \u2014 must be an AT URI (at://).');return}let n=Y(t);if(!n){this.renderError("Could not parse AT URI from link tag.");return}let r=await K(n.did),o=await Q(r,n);if(o?.bskyPostRef?.uri){this.rootPostUri=o.bskyPostRef.uri,this.rootPostCid=o.bskyPostRef.cid;let[a,i]=await Promise.all([C(this.config.apiUrl),S(o.bskyPostRef.uri)]);if(a){let s=await L(a.handle);this.currentUser={...a,...s}}this.threadData=i,E(i,this.viewerState),this.renderWidget()}else{this.documentAtUri=n,this.documentRecord=o,this.linkingStep="setup";let a=await C(this.config.apiUrl);if(a){let i=await L(a.handle);this.currentUser={...a,...i}}this.renderLinkingUI()}}catch(e){let t=e instanceof Error?e.message:"Unknown error";this.renderError(`Could not load comments: ${t}`)}}renderLoading(){this.container.innerHTML="";let e=this.makeRoot(),t=document.createElement("div");t.className="juttu-loading",t.textContent="Loading comments\u2026",e.appendChild(t),this.container.appendChild(e)}renderError(e){this.container.innerHTML="";let t=this.makeRoot(),n=document.createElement("div");n.className="juttu-error",n.textContent=`Juttu: ${e}`,t.appendChild(n),this.container.appendChild(t)}makeRoot(){let e=document.createElement("div");return e.className="juttu-comments",this.config.theme!=="auto"&&e.setAttribute("data-juttu-theme",this.config.theme),e}renderWidget(){this.container.innerHTML="";let e=this.makeRoot(),t=this.threadData?tt(this.threadData):[];e.appendChild(this.renderHeader(t.length)),e.appendChild(this.renderComposer()),e.appendChild(this.renderThread(t));let n=document.createElement("div");n.className="juttu-footer";let r=document.createElement("a");r.className="juttu-powered-by",r.href="https://juttu.app",r.target="_blank",r.rel="noopener noreferrer",r.textContent="Powered by Juttu",n.appendChild(r),e.appendChild(n),e.addEventListener("click",o=>this.handleClick(o)),e.addEventListener("input",o=>{let a=o.target;if(a.classList.contains("juttu-compose-input")){let i=e.querySelector(".juttu-submit-btn");i&&(i.disabled=!a.value.trim())}if(a.classList.contains("juttu-reply-input")){let s=a.closest(".juttu-reply-form")?.querySelector(".juttu-reply-submit");s&&(s.disabled=!a.value.trim())}}),this.container.appendChild(e)}handleClick(e){let t=e.target,n=t.closest(".juttu-sort-btn");if(n){let s=n.dataset.sort;s&&this.setSortOrder(s);return}if(t.closest(".juttu-load-more-btn")){this.pagination.visibleTopLevel+=10,this.renderWidget();return}let r=t.closest(".juttu-show-replies-btn");if(r){let s=r.dataset.uri;if(s){let l=this.pagination.visibleReplies.get(s)??3;this.pagination.visibleReplies.set(s,l+3),this.renderWidget()}return}if(t.closest(".juttu-login-link")){this.openLoginPopup();return}if(t.closest(".juttu-logout-btn")){this.handleLogout();return}let o=t.closest(".juttu-like-btn");if(o){this.requireAuth()&&this.handleLike(o);return}let a=t.closest(".juttu-repost-btn");if(a){this.requireAuth()&&this.handleRepost(a);return}let i=t.closest(".juttu-reply-btn");if(i){let s=i.dataset.uri;s&&this.handleToggleReplyForm(s);return}if(t.closest(".juttu-reply-cancel")){this.closeReplyForm();return}if(t.closest(".juttu-reply-submit")){this.openReplyFormUri&&this.requireAuth()&&this.handleSubmitReply(this.openReplyFormUri);return}if(t.closest(".juttu-submit-btn")){this.requireAuth()&&this.handlePost();return}}setSortOrder(e){this.sortOrder=e,this.pagination.visibleTopLevel=10,this.pagination.visibleReplies.clear(),this.renderWidget()}renderHeader(e){let t=document.createElement("div");t.className="juttu-header";let n=document.createElement("h2");return n.className="juttu-title",n.textContent=`${e} Comment${e!==1?"s":""}`,t.appendChild(n),t}makeSortControls(){let e=document.createElement("div");e.className="juttu-sort-controls";for(let{value:t,label:n}of[{value:"newest",label:"Newest"},{value:"oldest",label:"Oldest"},{value:"most-liked",label:"Top"}]){let r=document.createElement("button");r.className="juttu-sort-btn"+(this.sortOrder===t?" juttu-sort-btn--active":""),r.dataset.sort=t,r.textContent=n,e.appendChild(r)}return e}renderComposer(){let e=document.createElement("div");return e.className="juttu-composer",e.appendChild(this.makeComposeArea()),e}makeComposeArea(){let e=document.createElement("div");if(e.className="juttu-compose-area",this.currentUser){let i=document.createElement("div");if(i.className="juttu-compose-user",this.currentUser.avatar){let d=document.createElement("img");d.className="juttu-compose-avatar",d.src=this.currentUser.avatar,d.alt=this.currentUser.handle,i.appendChild(d)}else{let d=document.createElement("div");d.className="juttu-avatar-placeholder",i.appendChild(d)}let s=document.createElement("div");if(s.className="juttu-author-info",this.currentUser.displayName){let d=document.createElement("span");d.className="juttu-display-name",d.textContent=this.currentUser.displayName,s.appendChild(d)}let l=document.createElement("span");l.className="juttu-handle",l.textContent=`@${this.currentUser.handle}`,s.appendChild(l),i.appendChild(s);let c=document.createElement("button");c.className="juttu-logout-btn",c.textContent="Logout",i.appendChild(c),e.appendChild(i)}else{let i=document.createElement("button");i.className="juttu-login-link",i.textContent="Login to comment",e.appendChild(i)}let t=document.createElement("textarea");t.className="juttu-compose-input",t.placeholder="Write a comment\u2026",t.rows=3;let{wrap:n,backdrop:r}=this.buildEditorWrap(t);e.appendChild(n),setTimeout(()=>this.syncBackdropStyles(r,t),0),t.addEventListener("input",()=>{this.updateBackdrop(r,t.value),this.handleMentionInput(t)}),t.addEventListener("scroll",()=>this.syncScroll(r,t)),t.addEventListener("keydown",i=>{i.key==="Escape"&&this.dismissAutocomplete(t)});let o=document.createElement("div");o.className="juttu-compose-actions",o.appendChild(this.makeSortControls());let a=document.createElement("button");return a.className="juttu-submit-btn",a.textContent="Post comment",a.disabled=!0,o.appendChild(a),e.appendChild(o),e}renderThread(e){let t=document.createElement("div");t.className="juttu-thread";let n=z(e,this.sortOrder),r=n.slice(0,this.pagination.visibleTopLevel);for(let o of r)t.appendChild(this.renderComment(o,0));if(n.length>this.pagination.visibleTopLevel){let o=document.createElement("div");o.className="juttu-load-more";let a=document.createElement("button");a.className="juttu-load-more-btn";let i=n.length-this.pagination.visibleTopLevel;a.textContent=`Load ${Math.min(i,10)} more comments`,o.appendChild(a),t.appendChild(o)}return t}renderComment(e,t){let{post:n}=e,r=n.uri.split("/").pop()??"",o=`https://bsky.app/profile/${n.author.did}/post/${r}`,a=`https://bsky.app/profile/${n.author.handle}`,i=document.createElement("div");i.className="juttu-comment",i.dataset.uri=n.uri,i.dataset.cid=n.cid;let s=document.createElement("div");if(s.className="juttu-comment-header",n.author.avatar){let m=document.createElement("a");m.className="juttu-avatar-link",m.href=a,m.target="_blank",m.rel="noopener noreferrer";let f=document.createElement("img");f.className="juttu-avatar",f.src=n.author.avatar,f.alt=n.author.handle,f.loading="lazy",m.appendChild(f),s.appendChild(m)}else{let m=document.createElement("div");m.className="juttu-avatar-placeholder",s.appendChild(m)}let l=document.createElement("div");l.className="juttu-author-info";let c=document.createElement("a");c.className="juttu-display-name",c.href=a,c.target="_blank",c.rel="noopener noreferrer",c.textContent=n.author.displayName||n.author.handle,l.appendChild(c);let d=document.createElement("a");d.className="juttu-handle",d.href=a,d.target="_blank",d.rel="noopener noreferrer",d.textContent=`@${n.author.handle}`,l.appendChild(d),s.appendChild(l);let h=document.createElement("a");h.className="juttu-time-link",h.href=o,h.target="_blank",h.rel="noopener noreferrer",h.textContent=I(n.indexedAt),h.title=new Date(n.indexedAt).toLocaleString(),s.appendChild(h),i.appendChild(s);let p=document.createElement("div");p.className="juttu-comment-body";let M=document.createElement("p");if(M.style.margin="0",M.appendChild(X(n.record.text,n.record.facets)),p.appendChild(M),i.appendChild(p),n.embed?.$type==="app.bsky.embed.images#view"&&n.embed.images?.length){let m=document.createElement("div");m.className="juttu-comment-images";for(let f of n.embed.images){let g=document.createElement("a");g.className="juttu-comment-image-link",g.href=o,g.target="_blank",g.rel="noopener noreferrer";let j=document.createElement("img");j.className="juttu-comment-image",j.src=f.thumb,j.alt=f.alt||"Embedded image",j.loading="lazy",g.appendChild(j),m.appendChild(g)}i.appendChild(m)}let B=this.viewerState.get(n.uri)??{},rt=!!B.likeUri,ot=!!B.repostUri,y=document.createElement("div");y.className="juttu-comment-actions";let v=document.createElement("button");v.className="juttu-like-btn",v.dataset.liked=String(rt),v.dataset.uri=n.uri,v.dataset.cid=n.cid,v.title="Like",v.innerHTML=_;let U=document.createElement("span");U.className="juttu-like-count",U.textContent=String(n.likeCount??0),v.appendChild(U),y.appendChild(v);let b=document.createElement("button");b.className="juttu-repost-btn",b.dataset.reposted=String(ot),b.dataset.uri=n.uri,b.dataset.cid=n.cid,b.title="Repost",b.innerHTML=O;let A=document.createElement("span");A.className="juttu-repost-count",A.textContent=String(n.repostCount??0),b.appendChild(A),y.appendChild(b);let x=document.createElement("button");x.className="juttu-reply-btn",x.dataset.uri=n.uri,x.title="Reply",x.innerHTML=V;let R=document.createElement("span");R.className="juttu-reply-count",R.textContent=String(n.replyCount??0),x.appendChild(R),y.appendChild(x);let k=document.createElement("a");k.className="juttu-bsky-link",k.href=o,k.target="_blank",k.rel="noopener noreferrer",k.title="View on Bluesky",k.innerHTML=F,y.appendChild(k),i.appendChild(y);let $=(e.replies??[]).filter(m=>m.$type==="app.bsky.feed.defs#threadViewPost");if($.length>0){let m=document.createElement("div");m.className="juttu-replies";let f=this.pagination.visibleReplies.get(n.uri)??3,g=z($,this.sortOrder);for(let w of g.slice(0,f))m.appendChild(this.renderComment(w,t+1));let j=g.length-f;if(j>0){let w=document.createElement("button");w.className="juttu-show-replies-btn",w.dataset.uri=n.uri,w.textContent=`Show ${Math.min(j,3)} more repl${j===1?"y":"ies"}`,m.appendChild(w)}i.appendChild(m)}return i}getComposer(){return this.container.querySelector(".juttu-composer")??this.container.querySelector(".juttu-linking")}openLoginPopup(){if(this.loginPollInterval!==null){try{this.loginPopup?.focus()}catch{}return}this.loginPopup=window.open(`${this.config.apiUrl}/login`,"juttu-auth","width=500,height=600,menubar=no,toolbar=no,location=no,status=no"),this.loginPollStartTime=Date.now(),this.loginPollInterval=setInterval(()=>this.pollForLogin(),1500)}async onAuthComplete(){let e=await C(this.config.apiUrl);e&&await this.completeLogin(e)}async pollForLogin(){if(Date.now()-this.loginPollStartTime>12e4){this.cancelLogin();return}let t=await C(this.config.apiUrl);t?await this.completeLogin(t):this.loginPopup?.closed&&this.cancelLogin()}async completeLogin(e){this.loginPollInterval!==null&&(clearInterval(this.loginPollInterval),this.loginPollInterval=null);try{this.loginPopup?.close()}catch{}this.loginPopup=null;let t=await L(e.handle);if(this.currentUser={...e,...t},this.documentAtUri)this.linkingStep=this.documentRecord?"choose-method":"metadata",this.renderLinkingUI();else{let n=this.getComposer();n&&(this.clearMentionState(),n.innerHTML="",n.appendChild(this.makeComposeArea()))}}cancelLogin(){this.loginPollInterval!==null&&(clearInterval(this.loginPollInterval),this.loginPollInterval=null);try{this.loginPopup?.close()}catch{}if(this.loginPopup=null,this.documentAtUri)this.linkingStep="login",this.renderLinkingUI();else{let e=this.getComposer();e&&(this.clearMentionState(),e.innerHTML="",e.appendChild(this.makeComposeArea()))}}async handleLogout(){try{await fetch(`${this.config.apiUrl}/auth/logout`,{method:"POST",credentials:"include"})}catch{}this.currentUser=null,this.viewerState.clear();let e=this.getComposer();e&&(this.clearMentionState(),e.innerHTML="",e.appendChild(this.makeComposeArea()))}requireAuth(){return this.currentUser?!0:(this.openLoginPopup(),!1)}async handleLike(e){let t=e.dataset.uri,n=e.dataset.cid;if(!t||!n)return;let r=this.viewerState.get(t)??{},o=!!r.likeUri,a=e.querySelector(".juttu-like-count"),i=parseInt(a?.textContent??"0",10),s=!o;e.dataset.liked=String(s),a&&(a.textContent=String(i+(s?1:-1))),this.viewerState.set(t,{...r,likeUri:s?"pending":void 0});try{if(o&&r.likeUri){let l=r.likeUri.split("/").pop();await fetch(`${this.config.apiUrl}/bsky/like`,{method:"DELETE",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({rkey:l})}).then(this.checkApiResponse),this.viewerState.set(t,{...r,likeUri:void 0})}else{let c=await(await fetch(`${this.config.apiUrl}/bsky/like`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({uri:t,cid:n})}).then(this.checkApiResponse)).json();this.viewerState.set(t,{...r,likeUri:c.uri})}}catch(l){e.dataset.liked=String(o),a&&(a.textContent=String(i)),this.viewerState.set(t,r),this.showActionError(e,l instanceof Error?l.message:"Action failed")}}async handleRepost(e){let t=e.dataset.uri,n=e.dataset.cid;if(!t||!n)return;let r=this.viewerState.get(t)??{},o=!!r.repostUri,a=e.querySelector(".juttu-repost-count"),i=parseInt(a?.textContent??"0",10),s=!o;e.dataset.reposted=String(s),a&&(a.textContent=String(i+(s?1:-1))),this.viewerState.set(t,{...r,repostUri:s?"pending":void 0});try{if(o&&r.repostUri){let l=r.repostUri.split("/").pop();await fetch(`${this.config.apiUrl}/bsky/repost`,{method:"DELETE",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({rkey:l})}).then(this.checkApiResponse),this.viewerState.set(t,{...r,repostUri:void 0})}else{let c=await(await fetch(`${this.config.apiUrl}/bsky/repost`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({uri:t,cid:n})}).then(this.checkApiResponse)).json();this.viewerState.set(t,{...r,repostUri:c.uri})}}catch(l){e.dataset.reposted=String(o),a&&(a.textContent=String(i)),this.viewerState.set(t,r),this.showActionError(e,l instanceof Error?l.message:"Action failed")}}handleToggleReplyForm(e){if(this.openReplyFormUri===e){this.closeReplyForm();return}this.closeReplyForm();let t=this.container.querySelector(`.juttu-comment[data-uri="${CSS.escape(e)}"]`);if(!t)return;let n=document.createElement("div");n.className="juttu-reply-form";let r=document.createElement("textarea");r.className="juttu-reply-input",r.placeholder="Write a reply\u2026",r.rows=2;let{wrap:o,backdrop:a}=this.buildEditorWrap(r);n.appendChild(o),setTimeout(()=>this.syncBackdropStyles(a,r),0),r.addEventListener("input",()=>{this.updateBackdrop(a,r.value),this.handleMentionInput(r)}),r.addEventListener("scroll",()=>this.syncScroll(a,r)),r.addEventListener("keydown",h=>{h.key==="Escape"&&this.dismissAutocomplete(r)});let i=document.createElement("div");i.className="juttu-reply-form-actions";let s=document.createElement("span");s.className="juttu-reply-hint",s.textContent="Reply posts to Bluesky",i.appendChild(s);let l=document.createElement("button");l.className="juttu-reply-cancel",l.textContent="Cancel",i.appendChild(l);let c=document.createElement("button");c.className="juttu-reply-submit",c.textContent="Reply",c.disabled=!0,i.appendChild(c),n.appendChild(i);let d=t.querySelector(".juttu-comment-actions");d?.nextSibling?t.insertBefore(n,d.nextSibling):t.appendChild(n),this.openReplyFormUri=e,r.focus()}closeReplyForm(){if(!this.openReplyFormUri)return;this.container.querySelector(".juttu-reply-form")?.remove(),this.openReplyFormUri=null}async handlePost(){let e=this.container.querySelector(".juttu-compose-input"),t=this.container.querySelector(".juttu-submit-btn");if(!e||!t)return;let n=e.value.trim();if(!(!n||!this.rootPostUri||!this.rootPostCid)){t.disabled=!0,t.textContent="Posting\u2026",this.container.querySelector(".juttu-post-error")?.remove();try{await fetch(`${this.config.apiUrl}/bsky/post`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({text:n,reply:{root:{uri:this.rootPostUri,cid:this.rootPostCid},parent:{uri:this.rootPostUri,cid:this.rootPostCid}}})}).then(this.checkApiResponse),e.value="";let r=e.closest(".juttu-editor-wrap")?.querySelector(".juttu-editor-backdrop");r&&this.updateBackdrop(r,""),t.disabled=!0,t.textContent="Post comment",setTimeout(()=>this.refetchAndRender(),1500)}catch(r){t.disabled=!1,t.textContent="Post comment";let o=document.createElement("div");o.className="juttu-post-error",o.textContent=r instanceof Error?r.message:"Failed to post",t.insertAdjacentElement("afterend",o)}}}async handleSubmitReply(e){let t=this.container.querySelector(".juttu-reply-form");if(!t)return;let n=t.querySelector(".juttu-reply-input"),r=t.querySelector(".juttu-reply-submit");if(!n||!r)return;let o=n.value.trim();if(!o||!this.rootPostUri||!this.rootPostCid)return;let a=D(this.threadData,e);if(a){r.disabled=!0,r.textContent="Replying\u2026",t.querySelector(".juttu-reply-error")?.remove();try{await fetch(`${this.config.apiUrl}/bsky/post`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({text:o,reply:{root:{uri:this.rootPostUri,cid:this.rootPostCid},parent:{uri:a.uri,cid:a.cid}}})}).then(this.checkApiResponse),this.closeReplyForm(),setTimeout(()=>this.refetchAndRender(),1500)}catch(i){r.disabled=!1,r.textContent="Reply";let s=document.createElement("div");s.className="juttu-reply-error",s.textContent=i instanceof Error?i.message:"Failed to post reply",t.appendChild(s)}}}async refetchAndRender(){if(this.rootPostUri)try{let e=await S(this.rootPostUri);this.threadData=e;let t=new Map;E(e,t);for(let[n,r]of t){let o=this.viewerState.get(n),a={likeUri:o?.likeUri!=="pending"?o?.likeUri??r.likeUri:r.likeUri,repostUri:o?.repostUri!=="pending"?o?.repostUri??r.repostUri:r.repostUri};(a.likeUri||a.repostUri)&&t.set(n,a)}this.viewerState=t,this.renderWidget()}catch{}}buildEditorWrap(e){let t=document.createElement("div");t.className="juttu-editor-wrap";let n=document.createElement("div");return n.className="juttu-editor-backdrop",n.setAttribute("aria-hidden","true"),t.appendChild(n),t.appendChild(e),{wrap:t,backdrop:n}}syncBackdropStyles(e,t){let n=window.getComputedStyle(t),r=e.style;for(let o of["paddingTop","paddingRight","paddingBottom","paddingLeft","borderTopWidth","borderRightWidth","borderBottomWidth","borderLeftWidth","fontFamily","fontSize","fontWeight","lineHeight","letterSpacing"])r[o]=n[o]}updateBackdrop(e,t){for(;e.firstChild;)e.removeChild(e.firstChild);for(let n of et(t))if(n.type==="plain")e.appendChild(document.createTextNode(n.text));else{let r=document.createElement("span");r.className=`juttu-editor-highlight-${n.type}`,r.textContent=n.text,e.appendChild(r)}e.appendChild(document.createTextNode(`
`))}syncScroll(e,t){e.scrollTop=t.scrollTop}clearMentionState(){this.mentionDebounceTimer!==null&&(clearTimeout(this.mentionDebounceTimer),this.mentionDebounceTimer=null),this.activeSuggestionsTextarea=null}handleMentionInput(e){let t=e.selectionStart??e.value.length,r=e.value.slice(0,t).match(/@([a-zA-Z0-9][a-zA-Z0-9.-]*)$/);r?(this.activeSuggestionsTextarea=e,this.mentionDebounceTimer!==null&&clearTimeout(this.mentionDebounceTimer),this.mentionDebounceTimer=setTimeout(()=>{this.fetchMentionSuggestions(r[1],e)},300)):this.dismissAutocomplete(e)}async fetchMentionSuggestions(e,t){if(this.activeSuggestionsTextarea===t)try{let n=await fetch(`https://public.api.bsky.app/xrpc/app.bsky.actor.searchActorsTypeahead?q=${encodeURIComponent(e)}&limit=4`);if(!n.ok||this.activeSuggestionsTextarea!==t)return;let r=await n.json();this.renderAutocomplete(r.actors,t)}catch{this.dismissAutocomplete(t)}}renderAutocomplete(e,t){let n=t.closest(".juttu-editor-wrap");if(!n||(n.querySelector(".juttu-autocomplete")?.remove(),e.length===0))return;let r=document.createElement("ul");r.className="juttu-autocomplete";for(let o of e){let a=document.createElement("li"),i=document.createElement("button");if(i.className="juttu-autocomplete-item",i.type="button",o.avatar){let c=document.createElement("img");c.className="juttu-autocomplete-avatar",c.src=o.avatar,c.alt="",i.appendChild(c)}else{let c=document.createElement("div");c.className="juttu-autocomplete-avatar",c.style.background="var(--juttu-border-color)",i.appendChild(c)}let s=document.createElement("div");if(o.displayName){let c=document.createElement("div");c.style.fontWeight="600",c.textContent=o.displayName,s.appendChild(c)}let l=document.createElement("div");l.className="juttu-autocomplete-handle",l.textContent=`@${o.handle}`,s.appendChild(l),i.appendChild(s),i.addEventListener("mousedown",c=>{c.preventDefault(),this.applyMentionSuggestion(o.handle,t)}),a.appendChild(i),r.appendChild(a)}n.appendChild(r)}applyMentionSuggestion(e,t){let n=t.selectionStart??t.value.length,r=t.value.slice(0,n).replace(/@([a-zA-Z0-9][a-zA-Z0-9.-]*)$/,`@${e} `);t.value=r+t.value.slice(n);let o=r.length;setTimeout(()=>{t.focus(),t.setSelectionRange(o,o)},0),this.dismissAutocomplete(t);let a=t.closest(".juttu-editor-wrap")?.querySelector(".juttu-editor-backdrop");a&&this.updateBackdrop(a,t.value)}dismissAutocomplete(e){this.mentionDebounceTimer!==null&&(clearTimeout(this.mentionDebounceTimer),this.mentionDebounceTimer=null),this.activeSuggestionsTextarea=null,e.closest(".juttu-editor-wrap")?.querySelector(".juttu-autocomplete")?.remove()}checkApiResponse=async e=>{if(!e.ok){let t=await e.json().catch(()=>({}));throw new Error(t.error??`Request failed (${e.status})`)}return e};showActionError(e,t){e.parentElement?.querySelector(".juttu-action-error")?.remove();let n=document.createElement("div");n.className="juttu-action-error",n.textContent=t,e.insertAdjacentElement("afterend",n),setTimeout(()=>n.remove(),4e3)}renderLinkingUI(){this.container.innerHTML="";let e=this.makeRoot(),t=document.createElement("div");switch(t.className="juttu-linking",this.linkingStep){case"setup":t.appendChild(this.makeLinkingSetup());break;case"login":t.appendChild(this.makeLinkingLoginForm());break;case"metadata":t.appendChild(this.makeLinkingMetadata());break;case"choose-method":t.appendChild(this.makeLinkingChooseMethod());break;case"write-post":t.appendChild(this.makeLinkingWritePost());break;case"select-post":t.appendChild(this.makeLinkingSelectPost());break}e.appendChild(t),e.addEventListener("click",o=>this.handleLinkingClick(o)),e.addEventListener("keydown",o=>{o.target.classList.contains("juttu-handle-input")&&o.key==="Enter"&&this.handleLoginSubmit()});let n=document.createElement("div");n.className="juttu-footer";let r=document.createElement("a");r.className="juttu-powered-by",r.href="https://juttu.app",r.target="_blank",r.rel="noopener noreferrer",r.textContent="Powered by Juttu",n.appendChild(r),e.appendChild(n),this.container.appendChild(e)}makeLinkingSetup(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title";let n=document.createElement("p");n.className="juttu-linking-desc",this.documentRecord?(t.textContent="Comments not linked yet",n.textContent="Link this article to a Bluesky post to enable the comment thread."):(t.textContent="Comments not set up yet",n.textContent="Set up a Bluesky-powered comment thread for this article."),e.appendChild(t),e.appendChild(n);let r=document.createElement("button");return r.className="juttu-linking-start-btn",r.textContent=this.documentRecord?"Link comments":"Set up comments",e.appendChild(r),e}makeLinkingLoginForm(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Sign in as the document owner",e.appendChild(t);let n=document.createElement("button");return n.className="juttu-linking-login-btn",n.textContent="Login with Bluesky \u2192",e.appendChild(n),e}makeLinkingMetadata(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Article details",e.appendChild(t);let n=document.createElement("div");n.className="juttu-linking-field";let r=document.createElement("label");r.className="juttu-linking-label",r.textContent="Title *",n.appendChild(r);let o=document.createElement("input");o.type="text",o.className="juttu-linking-input juttu-linking-title-input",o.placeholder="Article title",o.value=this.linkingTitle,n.appendChild(o),e.appendChild(n);let a=document.createElement("div");a.className="juttu-linking-field";let i=document.createElement("label");i.className="juttu-linking-label",i.textContent="Description (optional)",a.appendChild(i);let s=document.createElement("input");s.type="text",s.className="juttu-linking-input juttu-linking-desc-input",s.placeholder="Short description",s.value=this.linkingDescription,a.appendChild(s),e.appendChild(a);let l=document.createElement("button");return l.className="juttu-linking-continue-btn",l.textContent="Continue",l.disabled=!this.linkingTitle.trim(),e.appendChild(l),o.addEventListener("input",()=>{l.disabled=!o.value.trim()}),e}makeLinkingChooseMethod(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Link a Bluesky post",e.appendChild(t);let n=document.createElement("p");n.className="juttu-linking-desc",n.textContent="This post becomes the root of the comment thread.",e.appendChild(n);let r=document.createElement("div");r.className="juttu-linking-methods";let o=document.createElement("button");o.className="juttu-linking-method-btn",o.dataset.method="write";let a=document.createElement("div");a.className="juttu-linking-method-title",a.textContent="Write a new post";let i=document.createElement("div");i.className="juttu-linking-method-desc",i.textContent="Compose a post to announce this article",o.appendChild(a),o.appendChild(i),r.appendChild(o);let s=document.createElement("button");s.className="juttu-linking-method-btn",s.dataset.method="select";let l=document.createElement("div");l.className="juttu-linking-method-title",l.textContent="Use an existing post";let c=document.createElement("div");return c.className="juttu-linking-method-desc",c.textContent="Pick from your recent Bluesky posts",s.appendChild(l),s.appendChild(c),r.appendChild(s),e.appendChild(r),e}makeLinkingWritePost(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Write a post",e.appendChild(t);let n=document.createElement("textarea");n.className="juttu-linking-textarea",n.placeholder="Share this article on Bluesky\u2026",e.appendChild(n);let r=document.createElement("div");r.style.cssText="display:flex;gap:0.5rem;margin-top:0.5rem;";let o=document.createElement("button");o.className="juttu-linking-back-btn",o.style.cssText="background:none;border:1px solid var(--juttu-border-color);border-radius:var(--juttu-radius);padding:0.4rem 0.75rem;cursor:pointer;font-size:0.875rem;color:var(--juttu-text-muted);font-family:var(--juttu-font-family);",o.textContent="\u2190 Back",r.appendChild(o);let a=document.createElement("button");return a.className="juttu-linking-continue-btn juttu-linking-write-submit",a.textContent="Post & Link",a.disabled=!0,r.appendChild(a),e.appendChild(r),n.addEventListener("input",()=>{a.disabled=!n.value.trim()}),e}makeLinkingSelectPost(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Select a post",e.appendChild(t);let n=document.createElement("button");n.className="juttu-linking-back-btn",n.style.cssText="background:none;border:1px solid var(--juttu-border-color);border-radius:var(--juttu-radius);padding:0.3rem 0.65rem;cursor:pointer;font-size:0.8rem;color:var(--juttu-text-muted);font-family:var(--juttu-font-family);margin-bottom:0.75rem;",n.textContent="\u2190 Back",e.appendChild(n);let r=document.createElement("div");if(r.className="juttu-linking-post-list",this.userPosts.length===0){let o=document.createElement("p");o.className="juttu-linking-spinner",o.textContent="Loading posts\u2026",r.appendChild(o)}else for(let o of this.userPosts){let a=document.createElement("button");a.className="juttu-linking-post-item",a.dataset.uri=o.uri,a.dataset.cid=o.cid;let i=document.createElement("div");i.className="juttu-linking-post-text",i.textContent=o.text.length>180?o.text.slice(0,180)+"\u2026":o.text;let s=document.createElement("div");s.className="juttu-linking-post-date",s.textContent=I(o.createdAt),a.appendChild(i),a.appendChild(s),r.appendChild(a)}return e.appendChild(r),e}handleLinkingClick(e){let t=e.target;if(t.closest(".juttu-linking-start-btn")){this.currentUser?this.documentRecord?this.linkingStep="choose-method":this.linkingStep="metadata":this.linkingStep="login",this.renderLinkingUI();return}if(t.closest(".juttu-linking-login-btn")){this.openLoginPopup();return}if(t.closest(".juttu-linking-continue-btn")&&!t.closest(".juttu-linking-write-submit")){let o=this.container.querySelector(".juttu-linking-title-input"),a=this.container.querySelector(".juttu-linking-desc-input"),i=o?.value.trim()??"";if(!i)return;this.linkingTitle=i,this.linkingDescription=a?.value.trim()??"",this.linkingStep="choose-method",this.renderLinkingUI();return}let n=t.closest(".juttu-linking-method-btn");if(n){n.dataset.method==="write"?(this.linkingStep="write-post",this.renderLinkingUI()):n.dataset.method==="select"&&(this.linkingStep="select-post",this.userPosts=[],this.renderLinkingUI(),this.fetchUserPostsAndRender());return}if(t.closest(".juttu-linking-write-submit")){let a=this.container.querySelector(".juttu-linking-textarea")?.value.trim()??"";if(!a)return;this.handleLinkingCreatePost(a);return}let r=t.closest(".juttu-linking-post-item");if(r){let o=r.dataset.uri,a=r.dataset.cid;o&&a&&this.callPutDocument(o,a);return}if(t.closest(".juttu-linking-back-btn")){this.linkingStep="choose-method",this.renderLinkingUI();return}}async fetchUserPostsAndRender(){if(this.currentUser)try{let e=await fetch(`https://public.api.bsky.app/xrpc/app.bsky.feed.getAuthorFeed?actor=${encodeURIComponent(this.currentUser.handle)}&filter=posts_no_replies&limit=20`);if(!e.ok)throw new Error(`Failed to fetch posts: ${e.status}`);let t=await e.json();this.userPosts=t.feed.filter(n=>!n.reason).map(n=>({uri:n.post.uri,cid:n.post.cid,text:n.post.record.text,createdAt:n.post.record.createdAt})),this.linkingStep==="select-post"&&this.renderLinkingUI()}catch(e){if(this.linkingStep==="select-post"){let t=this.container.querySelector(".juttu-linking-post-list");if(t){t.innerHTML="";let n=document.createElement("p");n.className="juttu-linking-error",n.textContent=e instanceof Error?e.message:"Failed to load posts",t.appendChild(n)}}}}async handleLinkingCreatePost(e){let t=this.container.querySelector(".juttu-linking-write-submit");t&&(t.disabled=!0,t.textContent="Posting\u2026");try{let r=await(await fetch(`${this.config.apiUrl}/bsky/post`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({text:e})}).then(this.checkApiResponse)).json();await this.callPutDocument(r.uri,r.cid)}catch(n){t&&(t.disabled=!1,t.textContent="Post & Link");let r=document.createElement("p");r.className="juttu-linking-error",r.textContent=n instanceof Error?n.message:"Failed to post",this.container.querySelector(".juttu-linking")?.appendChild(r)}}async callPutDocument(e,t){if(!this.documentAtUri)return;let n=this.container.querySelector(".juttu-linking");if(n){let i=document.createElement("p");i.className="juttu-linking-spinner",i.textContent="Linking\u2026",n.appendChild(i)}let r={uri:e,cid:t},o=new Date().toISOString(),a;this.documentRecord?a={...this.documentRecord,bskyPostRef:r,updatedAt:o}:a={$type:"site.standard.document",site:window.location.origin,title:this.linkingTitle,description:this.linkingDescription||void 0,path:window.location.pathname||void 0,publishedAt:o,updatedAt:o,bskyPostRef:r};try{await fetch(`${this.config.apiUrl}/atproto/document`,{method:"PUT",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({rkey:this.documentAtUri.rkey,record:a})}).then(this.checkApiResponse),this.documentAtUri=null,this.documentRecord=null,this.rootPostUri=e,this.rootPostCid=t;let i=await S(e);this.threadData=i,E(i,this.viewerState),this.renderWidget()}catch(i){let s=this.container.querySelector(".juttu-linking");s?.querySelector(".juttu-linking-spinner")?.remove();let l=document.createElement("p");l.className="juttu-linking-error",l.textContent=i instanceof Error?i.message:"Failed to link document",s?.appendChild(l)}}destroy(){this.loginPollInterval!==null&&clearInterval(this.loginPollInterval);try{this.loginPopup?.close()}catch{}this.container.innerHTML=""}};if(typeof window<"u"){let e=function(t){let n=t.getAttribute("data-api-url"),r=t.getAttribute("data-theme")??"auto";if(!n){console.error("Juttu: Missing data-api-url attribute on script tag");return}let o=document.getElementById("juttu-comments");if(!o){console.error('Juttu: No element with id="juttu-comments" found on this page');return}new T(o,{apiUrl:n,theme:r})};window.JuttuWidget=T;let u=document.currentScript;u&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>e(u)):e(u))}})();
