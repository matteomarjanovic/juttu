...
`;function Q(u){if(!u.startsWith("at://"))return null;let e=u.slice(5).split("/");if(e.length!==3)return null;let[t,n,r]=e;return!t||!n||!r?null:{did:t,collection:n,rkey:r}}async function X(u){let e;if(u.startsWith("did:plc:")){let n=await fetch(`https://plc.directory/${u}`);if(!n.ok)throw new Error(`Failed to resolve DID: ${n.status}`);e=await n.json()}else if(u.startsWith("did:web:")){let n=u.split(":").slice(2).join(":"),r=await fetch(`https://${n}/.well-known/did.json`);if(!r.ok)throw new Error(`Failed to resolve did:web DID: ${r.status}`);e=await r.json()}else throw new Error(`Unsupported DID method: ${u}`);let t=e.service?.find(n=>n.type==="AtprotoPersonalDataServer");if(!t?.serviceEndpoint)throw new Error("No PDS endpoint found in DID document");return t.serviceEndpoint}async function tt(u,e){let t=`${u}/xrpc/com.atproto.repo.getRecord?repo=${encodeURIComponent(e.did)}&collection=${encodeURIComponent(e.collection)}&rkey=${encodeURIComponent(e.rkey)}`,n=await fetch(t);if(n.status>=400&&n.status<500)return null;if(!n.ok)throw new Error(`Failed to fetch document record: ${n.status}`);return(await n.json()).value}async function S(u){let e=`https://public.api.bsky.app/xrpc/app.bsky.feed.getPostThread?uri=${encodeURIComponent(u)}&depth=10&parentHeight=0`,t=await fetch(e);if(!t.ok)throw new Error(`Failed to fetch thread: ${t.status}`);let n=await t.json();if(!n.thread)throw new Error("Thread data missing from response");return n.thread}async function I(u,e){try{let t=await fetch(`${u}/bsky/thread?uri=${encodeURIComponent(e)}`,{credentials:"include"});if(!t.ok)return new Map;let n=await t.json();return new Map(Object.entries(n.states))}catch{return new Map}}async function D(u){try{let e=await fetch(`${u}/auth/me`,{credentials:"include"});return e.ok?await e.json():null}catch{return null}}async function z(u){try{let e=await fetch(`https://public.api.bsky.app/xrpc/app.bsky.actor.getProfile?actor=${encodeURIComponent(u)}`);if(!e.ok)return{};let t=await e.json();return{avatar:t.avatar,displayName:t.displayName}}catch{return{}}}function $(u){let e=Math.floor((Date.now()-new Date(u).getTime())/1e3);return e<60?"just now":e<3600?`${Math.floor(e/60)}m`:e<86400?`${Math.floor(e/3600)}h`:e<2592e3?`${Math.floor(e/86400)}d`:e<31536e3?`${Math.floor(e/2592e3)}mo`:`${Math.floor(e/31536e3)}y`}function at(u){return u.startsWith("https://")||u.startsWith("http://")}function et(u,e){let t=document.createDocumentFragment(),n=new TextEncoder,r=new TextDecoder,o=n.encode(u);if(!e||e.length===0)return t.appendChild(document.createTextNode(u)),t;let s=[...e].sort((a,l)=>a.index.byteStart-l.index.byteStart),i=0;for(let a of s){let{byteStart:l,byteEnd:c}=a.index;l>i&&t.appendChild(document.createTextNode(r.decode(o.slice(i,l))));let d=r.decode(o.slice(l,c)),p=a.features[0];if(!p)t.appendChild(document.createTextNode(d));else if(p.$type==="app.bsky.richtext.facet#link")if(at(p.uri)){let m=document.createElement("a");m.className="juttu-link",m.href=p.uri,m.target="_blank",m.rel="noopener noreferrer",m.textContent=d,t.appendChild(m)}else t.appendChild(document.createTextNode(d));else if(p.$type==="app.bsky.richtext.facet#mention"){let m=document.createElement("a");m.className="juttu-mention",m.href=`https://bsky.app/profile/${p.did}`,m.target="_blank",m.rel="noopener noreferrer",m.textContent=d,t.appendChild(m)}else if(p.$type==="app.bsky.richtext.facet#tag"){let m=document.createElement("a");m.className="juttu-hashtag",m.href=`https://bsky.app/search?q=${encodeURIComponent("#"+p.tag)}`,m.target="_blank",m.rel="noopener noreferrer",m.textContent=d,t.appendChild(m)}else t.appendChild(document.createTextNode(d));i=c}return i<o.length&&t.appendChild(document.createTextNode(r.decode(o.slice(i)))),t}function nt(u){return(u.replies??[]).filter(e=>e.$type==="app.bsky.feed.defs#threadViewPost")}function B(u,e){return[...u].sort((t,n)=>e==="most-liked"?(n.post.likeCount??0)-(t.post.likeCount??0):e==="oldest"?new Date(t.post.indexedAt).getTime()-new Date(n.post.indexedAt).getTime():new Date(n.post.indexedAt).getTime()-new Date(t.post.indexedAt).getTime())}function x(u,e){let{post:t}=u;e.set(t.uri,{likeUri:t.viewer?.like,repostUri:t.viewer?.repost});for(let n of u.replies??[])n.$type==="app.bsky.feed.defs#threadViewPost"&&x(n,e)}function E(u,e){if(!u)return null;if(u.post.uri===e)return u.post;for(let t of u.replies??[])if(t.$type==="app.bsky.feed.defs#threadViewPost"){let n=E(t,e);if(n)return n}return null}var G=/@([a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+)/g,J=/https?:\/\/[^\s\x00-\x1f<>"]+/g,Y=/(^|[\s(])((?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}(?:\/[^\s\x00-\x1f<>"]*)?)/g,K=/#[^\s\x00-\x1f.,;!?\-#]{1,64}/g,H=/[.,;!?)\]]+$/;function lt(u){let t=u.split("/")[0].split(".").at(-1)??"";return t.length>=2&&/^[a-z]+$/i.test(t)}function rt(u){let e=[],t;for(J.lastIndex=0;(t=J.exec(u))!==null;){let i=t[0].replace(H,"");i&&e.push({start:t.index,end:t.index+i.length,type:"link"})}for(Y.lastIndex=0;(t=Y.exec(u))!==null;){let i=t[1]??"",a=t[2];if(!lt(a))continue;let l=t.index+i.length,c=a.replace(H,"");c&&e.push({start:l,end:l+c.length,type:"link"})}for(K.lastIndex=0;(t=K.exec(u))!==null;){let i=t[0].replace(H,"");i&&e.push({start:t.index,end:t.index+i.length,type:"tag"})}for(G.lastIndex=0;(t=G.exec(u))!==null;){let i=t.index;if(i>0){let a=u[i-1];if(a!==" "&&a!=="	"&&a!==`
`&&a!=="(")continue}e.push({start:i,end:i+t[0].length,type:"mention"})}e.sort((i,a)=>i.start-a.start);let n=[],r=0;for(let i of e)i.start<r||(n.push(i),r=i.end);let o=[],s=0;for(let i of n)i.start>s&&o.push({text:u.slice(s,i.start),type:"plain"}),o.push({text:u.slice(i.start,i.end),type:i.type}),s=i.end;return s<u.length&&o.push({text:u.slice(s),type:"plain"}),o}var T=class{constructor(e,t){this.threadData=null;this.currentUser=null;this.rootPostUri=null;this.rootPostCid=null;this.sortOrder="newest";this.pagination={visibleTopLevel:10,visibleReplies:new Map};this.viewerState=new Map;this.localCounts=new Map;this.pendingActions=new Set;this.openReplyFormUri=null;this.loginPopup=null;this.loginPollInterval=null;this.loginPollStartTime=0;this.popupClosedAt=0;this.pendingAction=null;this.mentionDebounceTimer=null;this.activeSuggestionsTextarea=null;this.documentAtUri=null;this.documentRecord=null;this.linkingStep="setup";this.linkingTitle="";this.linkingDescription="";this.userPosts=[];this.checkApiResponse=async e=>{if(!e.ok){let t=await e.json().catch(()=>({}));throw new Error(t.error??`Request failed (${e.status})`)}return e};this.container=e,this.config=t,this.injectStyles(),this.init()}injectStyles(){if(document.getElementById("juttu-styles"))return;let e=document.createElement("style");e.id="juttu-styles",e.textContent=Z,document.head.appendChild(e)}async init(){this.renderLoading();try{let e=document.querySelector('link[rel="site.standard.document"]');if(!e){this.renderError('Missing <link rel="site.standard.document"> tag on this page. Add it to enable comments.');return}let t=e.getAttribute("href");if(!t||!t.startsWith("at://")){this.renderError('Invalid <link rel="site.standard.document" href> \u2014 must be an AT URI (at://).');return}let n=Q(t);if(!n){this.renderError("Could not parse AT URI from link tag.");return}let r=await X(n.did),o=await tt(r,n);o?.bskyPostRef?.uri?(this.rootPostUri=o.bskyPostRef.uri,this.rootPostCid=o.bskyPostRef.cid,this.threadData=await S(o.bskyPostRef.uri),x(this.threadData,this.viewerState),this.syncLocalCounts(this.threadData),this.renderWidget()):(this.documentAtUri=n,this.documentRecord=o,this.linkingStep="setup",this.renderLinkingUI())}catch(e){let t=e instanceof Error?e.message:"Unknown error";this.renderError(`Could not load comments: ${t}`)}}syncLocalCounts(e){let t=n=>{let{post:r}=n;this.localCounts.set(r.uri,{likes:r.likeCount??0,reposts:r.repostCount??0,replies:r.replyCount??0});for(let o of n.replies??[])o.$type==="app.bsky.feed.defs#threadViewPost"&&t(o)};t(e)}renderLoading(){this.container.innerHTML="";let e=this.makeRoot(),t=document.createElement("div");t.className="juttu-loading",t.textContent="Loading comments\u2026",e.appendChild(t),this.container.appendChild(e)}renderError(e){this.container.innerHTML="";let t=this.makeRoot(),n=document.createElement("div");n.className="juttu-error",n.textContent=`Juttu: ${e}`,t.appendChild(n),this.container.appendChild(t)}detectTheme(){let e=document.createElement("div");this.container.appendChild(e);let t=window.getComputedStyle(e).color;this.container.removeChild(e);let n=t.match(/\d+/g);if(!n||n.length<3)return"light";let[r,o,s]=n.map(Number);return r<=120&&o<=120&&s<=120?"light":"dark"}makeRoot(){let e=document.createElement("div");e.className="juttu-comments";let t=this.config.theme==="auto"?this.detectTheme():this.config.theme;return e.setAttribute("data-juttu-theme",t),e}renderWidget(){this.container.innerHTML="";let e=this.makeRoot(),t=this.threadData?nt(this.threadData):[];e.appendChild(this.renderHeader(t.length)),e.appendChild(this.renderComposer()),e.appendChild(this.renderThread(t));let n=document.createElement("div");n.className="juttu-footer";let r=document.createElement("a");r.className="juttu-powered-by",r.href="https://juttu.app",r.target="_blank",r.rel="noopener noreferrer",r.textContent="Powered by Juttu",n.appendChild(r),e.appendChild(n),e.addEventListener("click",o=>this.handleClick(o)),e.addEventListener("input",o=>{let s=o.target;if(s.classList.contains("juttu-compose-input")){let i=e.querySelector(".juttu-submit-btn");i&&(i.disabled=!s.value.trim())}if(s.classList.contains("juttu-reply-input")){let a=s.closest(".juttu-reply-form")?.querySelector(".juttu-reply-submit");a&&(a.disabled=!s.value.trim())}}),this.container.appendChild(e)}handleClick(e){let t=e.target,n=t.closest(".juttu-sort-btn");if(n){let a=n.dataset.sort;a&&this.setSortOrder(a);return}if(t.closest(".juttu-load-more-btn")){this.pagination.visibleTopLevel+=10,this.renderWidget();return}let r=t.closest(".juttu-show-replies-btn");if(r){let a=r.dataset.uri;if(a){let l=this.pagination.visibleReplies.get(a)??3;this.pagination.visibleReplies.set(a,l+3),this.renderWidget()}return}if(t.closest(".juttu-login-link")){this.resolveAuth();return}if(t.closest(".juttu-logout-btn")){this.handleLogout();return}let o=t.closest(".juttu-like-btn");if(o){this.resolveAuth(()=>this.handleLike(o));return}let s=t.closest(".juttu-repost-btn");if(s){this.resolveAuth(()=>this.handleRepost(s));return}let i=t.closest(".juttu-reply-btn");if(i){let a=i.dataset.uri;a&&this.handleToggleReplyForm(a);return}if(t.closest(".juttu-reply-cancel")){this.closeReplyForm();return}if(t.closest(".juttu-reply-submit")){this.openReplyFormUri&&this.resolveAuth(()=>this.handleSubmitReply(this.openReplyFormUri));return}if(t.closest(".juttu-submit-btn")){this.resolveAuth(()=>this.handlePost());return}}setSortOrder(e){this.sortOrder=e,this.pagination.visibleTopLevel=10,this.pagination.visibleReplies.clear(),this.renderWidget()}renderHeader(e){let t=document.createElement("div");t.className="juttu-header";let n=document.createElement("h2");return n.className="juttu-title",n.textContent=`${e} Comment${e!==1?"s":""}`,t.appendChild(n),t}makeSortControls(){let e=document.createElement("div");e.className="juttu-sort-controls";for(let{value:t,label:n}of[{value:"newest",label:"Newest"},{value:"oldest",label:"Oldest"},{value:"most-liked",label:"Top"}]){let r=document.createElement("button");r.className="juttu-sort-btn"+(this.sortOrder===t?" juttu-sort-btn--active":""),r.dataset.sort=t,r.textContent=n,e.appendChild(r)}return e}renderComposer(){let e=document.createElement("div");return e.className="juttu-composer",e.appendChild(this.makeComposeArea()),e}makeComposeArea(){let e=document.createElement("div");if(e.className="juttu-compose-area",this.currentUser){let i=document.createElement("div");if(i.className="juttu-compose-user",this.currentUser.avatar){let d=document.createElement("img");d.className="juttu-compose-avatar",d.src=this.currentUser.avatar,d.alt=this.currentUser.handle,i.appendChild(d)}else{let d=document.createElement("div");d.className="juttu-avatar-placeholder",i.appendChild(d)}let a=document.createElement("div");if(a.className="juttu-author-info",this.currentUser.displayName){let d=document.createElement("span");d.className="juttu-display-name",d.textContent=this.currentUser.displayName,a.appendChild(d)}let l=document.createElement("span");l.className="juttu-handle",l.textContent=`@${this.currentUser.handle}`,a.appendChild(l),i.appendChild(a);let c=document.createElement("button");c.className="juttu-logout-btn",c.textContent="Logout",i.appendChild(c),e.appendChild(i)}else{let i=document.createElement("button");i.className="juttu-login-link",i.textContent="Login to comment",e.appendChild(i)}let t=document.createElement("textarea");t.className="juttu-compose-input",t.placeholder="Write a comment\u2026",t.rows=3;let{wrap:n,backdrop:r}=this.buildEditorWrap(t);e.appendChild(n),setTimeout(()=>this.syncBackdropStyles(r,t),0),t.addEventListener("input",()=>{this.updateBackdrop(r,t.value),this.handleMentionInput(t)}),t.addEventListener("scroll",()=>this.syncScroll(r,t)),t.addEventListener("keydown",i=>{i.key==="Escape"&&this.dismissAutocomplete(t)});let o=document.createElement("div");o.className="juttu-compose-actions",o.appendChild(this.makeSortControls());let s=document.createElement("button");return s.className="juttu-submit-btn",s.textContent="Post comment",s.disabled=!0,o.appendChild(s),e.appendChild(o),e}renderThread(e){let t=document.createElement("div");t.className="juttu-thread";let n=B(e,this.sortOrder),r=n.slice(0,this.pagination.visibleTopLevel);for(let o of r)t.appendChild(this.renderComment(o,0));if(n.length>this.pagination.visibleTopLevel){let o=document.createElement("div");o.className="juttu-load-more";let s=document.createElement("button");s.className="juttu-load-more-btn";let i=n.length-this.pagination.visibleTopLevel;s.textContent=`Load ${Math.min(i,10)} more comments`,o.appendChild(s),t.appendChild(o)}return t}renderComment(e,t){let{post:n}=e,r=n.uri.split("/").pop()??"",o=`https://bsky.app/profile/${n.author.did}/post/${r}`,s=`https://bsky.app/profile/${n.author.handle}`,i=document.createElement("div");i.className="juttu-comment",i.dataset.uri=n.uri,i.dataset.cid=n.cid;let a=document.createElement("div");if(a.className="juttu-comment-header",n.author.avatar){let h=document.createElement("a");h.className="juttu-avatar-link",h.href=s,h.target="_blank",h.rel="noopener noreferrer";let f=document.createElement("img");f.className="juttu-avatar",f.src=n.author.avatar,f.alt=n.author.handle,f.loading="lazy",h.appendChild(f),a.appendChild(h)}else{let h=document.createElement("div");h.className="juttu-avatar-placeholder",a.appendChild(h)}let l=document.createElement("div");l.className="juttu-author-info";let c=document.createElement("a");c.className="juttu-display-name",c.href=s,c.target="_blank",c.rel="noopener noreferrer",c.textContent=n.author.displayName||n.author.handle,l.appendChild(c);let d=document.createElement("a");d.className="juttu-handle",d.href=s,d.target="_blank",d.rel="noopener noreferrer",d.textContent=`@${n.author.handle}`,l.appendChild(d),a.appendChild(l);let p=document.createElement("a");p.className="juttu-time-link",p.href=o,p.target="_blank",p.rel="noopener noreferrer",p.textContent=$(n.indexedAt),p.title=new Date(n.indexedAt).toLocaleString(),a.appendChild(p),i.appendChild(a);let m=document.createElement("div");m.className="juttu-comment-body";let N=document.createElement("p");if(N.style.margin="0",N.appendChild(et(n.record.text,n.record.facets)),m.appendChild(N),i.appendChild(m),n.embed?.$type==="app.bsky.embed.images#view"&&n.embed.images?.length){let h=document.createElement("div");h.className="juttu-comment-images";for(let f of n.embed.images){let g=document.createElement("a");g.className="juttu-comment-image-link",g.href=o,g.target="_blank",g.rel="noopener noreferrer";let v=document.createElement("img");v.className="juttu-comment-image",v.src=f.thumb,v.alt=f.alt||"Embedded image",v.loading="lazy",g.appendChild(v),h.appendChild(g)}i.appendChild(h)}let V=this.viewerState.get(n.uri)??{},it=!!V.likeUri,st=!!V.repostUri,A=this.localCounts.get(n.uri)??{likes:n.likeCount??0,reposts:n.repostCount??0,replies:n.replyCount??0},y=document.createElement("div");y.className="juttu-comment-actions";let j=document.createElement("button");j.className="juttu-like-btn",j.dataset.liked=String(it),j.dataset.uri=n.uri,j.dataset.cid=n.cid,j.title="Like",j.innerHTML=O;let M=document.createElement("span");M.className="juttu-like-count",M.textContent=String(A.likes),j.appendChild(M),y.appendChild(j);let k=document.createElement("button");k.className="juttu-repost-btn",k.dataset.reposted=String(st),k.dataset.uri=n.uri,k.dataset.cid=n.cid,k.title="Repost",k.innerHTML=F;let U=document.createElement("span");U.className="juttu-repost-count",U.textContent=String(A.reposts),k.appendChild(U),y.appendChild(k);let w=document.createElement("button");w.className="juttu-reply-btn",w.dataset.uri=n.uri,w.title="Reply",w.innerHTML=W;let R=document.createElement("span");R.className="juttu-reply-count",R.textContent=String(A.replies),w.appendChild(R),y.appendChild(w);let b=document.createElement("a");b.className="juttu-bsky-link",b.href=o,b.target="_blank",b.rel="noopener noreferrer",b.title="View on Bluesky",b.innerHTML=q,y.appendChild(b),i.appendChild(y);let _=(e.replies??[]).filter(h=>h.$type==="app.bsky.feed.defs#threadViewPost");if(_.length>0){let h=document.createElement("div");h.className="juttu-replies";let f=this.pagination.visibleReplies.get(n.uri)??3,g=B(_,this.sortOrder);for(let C of g.slice(0,f))h.appendChild(this.renderComment(C,t+1));let v=g.length-f;if(v>0){let C=document.createElement("button");C.className="juttu-show-replies-btn",C.dataset.uri=n.uri,C.textContent=`Show ${Math.min(v,3)} more repl${v===1?"y":"ies"}`,h.appendChild(C)}i.appendChild(h)}return i}getComposer(){return this.container.querySelector(".juttu-composer")??this.container.querySelector(".juttu-linking")}openLoginPopup(){if(this.loginPollInterval!==null){try{this.loginPopup?.focus()}catch{}return}this.loginPopup=window.open(`${this.config.apiUrl}/login`,"juttu-auth","width=500,height=600,menubar=no,toolbar=no,location=no,status=no"),this.loginPollStartTime=Date.now(),this.popupClosedAt=0,this.loginPollInterval=setInterval(()=>this.pollForLogin(),1500)}async pollForLogin(){if(Date.now()-this.loginPollStartTime>12e4){this.cancelLogin();return}let t=await D(this.config.apiUrl);t?await this.completeLogin(t):this.loginPopup?.closed&&(this.popupClosedAt||(this.popupClosedAt=Date.now()),Date.now()-this.popupClosedAt>5e3&&this.cancelLogin())}async completeLogin(e){this.loginPollInterval!==null&&(clearInterval(this.loginPollInterval),this.loginPollInterval=null);try{this.loginPopup?.close()}catch{}this.loginPopup=null;let t=await z(e.handle);if(this.currentUser={...e,...t},this.documentAtUri)this.linkingStep=this.documentRecord?"choose-method":"metadata",this.renderLinkingUI();else{let n=this.getComposer();n&&(this.clearMentionState(),n.innerHTML="",n.appendChild(this.makeComposeArea()));let r=this.pendingAction;if(this.pendingAction=null,await r?.(),this.rootPostUri){let o=await I(this.config.apiUrl,this.rootPostUri);for(let[s,i]of o){let a=this.viewerState.get(s);a?.likeUri==="pending"||a?.repostUri==="pending"||this.viewerState.set(s,i)}this.renderWidget()}}}cancelLogin(){this.pendingAction=null,this.loginPollInterval!==null&&(clearInterval(this.loginPollInterval),this.loginPollInterval=null);try{this.loginPopup?.close()}catch{}if(this.loginPopup=null,this.documentAtUri)this.linkingStep="login",this.renderLinkingUI();else{let e=this.getComposer();e&&(this.clearMentionState(),e.innerHTML="",e.appendChild(this.makeComposeArea()))}}async handleLogout(){try{await fetch(`${this.config.apiUrl}/auth/logout`,{method:"POST",credentials:"include"})}catch{}this.currentUser=null,this.viewerState.clear(),this.clearMentionState(),this.renderWidget()}async resolveAuth(e){if(this.currentUser){await e?.();return}let t=await D(this.config.apiUrl);if(t){let n=await z(t.handle);if(this.currentUser={...t,...n},this.documentAtUri)this.linkingStep=this.documentRecord?"choose-method":"metadata",this.renderLinkingUI();else{let r=this.getComposer();if(r&&(this.clearMentionState(),r.innerHTML="",r.appendChild(this.makeComposeArea())),await e?.(),this.rootPostUri){let o=await I(this.config.apiUrl,this.rootPostUri);for(let[s,i]of o){let a=this.viewerState.get(s);a?.likeUri==="pending"||a?.repostUri==="pending"||this.viewerState.set(s,i)}this.renderWidget()}}}else this.documentAtUri?(this.linkingStep="login",this.renderLinkingUI()):(this.pendingAction=e??null,this.openLoginPopup())}async handleLike(e){let t=e.dataset.uri,n=e.dataset.cid;if(!t||!n)return;let r=`like:${t}`;if(this.pendingActions.has(r))return;this.pendingActions.add(r);let o=this.viewerState.get(t)??{},s=!!o.likeUri,i=this.localCounts.get(t),a=i?.likes??0,l=!s,c=Math.max(0,a+(l?1:-1));e.dataset.liked=String(l);let d=e.querySelector(".juttu-like-count");d&&(d.textContent=String(c)),i&&(i.likes=c),this.viewerState.set(t,{...o,likeUri:l?"pending":void 0});try{if(s&&o.likeUri){let p=o.likeUri.split("/").pop();await fetch(`${this.config.apiUrl}/bsky/like`,{method:"DELETE",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({rkey:p})}).then(this.checkApiResponse),this.viewerState.set(t,{...o,likeUri:void 0})}else{let m=await(await fetch(`${this.config.apiUrl}/bsky/like`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({uri:t,cid:n})}).then(this.checkApiResponse)).json();this.viewerState.set(t,{...o,likeUri:m.uri})}}catch(p){e.dataset.liked=String(s),i&&(i.likes=a),d&&(d.textContent=String(a)),this.viewerState.set(t,o),this.showActionError(e,p instanceof Error?p.message:"Action failed")}finally{this.pendingActions.delete(r)}}async handleRepost(e){let t=e.dataset.uri,n=e.dataset.cid;if(!t||!n)return;let r=`repost:${t}`;if(this.pendingActions.has(r))return;this.pendingActions.add(r);let o=this.viewerState.get(t)??{},s=!!o.repostUri,i=this.localCounts.get(t),a=i?.reposts??0,l=!s,c=Math.max(0,a+(l?1:-1));e.dataset.reposted=String(l);let d=e.querySelector(".juttu-repost-count");d&&(d.textContent=String(c)),i&&(i.reposts=c),this.viewerState.set(t,{...o,repostUri:l?"pending":void 0});try{if(s&&o.repostUri){let p=o.repostUri.split("/").pop();await fetch(`${this.config.apiUrl}/bsky/repost`,{method:"DELETE",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({rkey:p})}).then(this.checkApiResponse),this.viewerState.set(t,{...o,repostUri:void 0})}else{let m=await(await fetch(`${this.config.apiUrl}/bsky/repost`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({uri:t,cid:n})}).then(this.checkApiResponse)).json();this.viewerState.set(t,{...o,repostUri:m.uri})}}catch(p){e.dataset.reposted=String(s),i&&(i.reposts=a),d&&(d.textContent=String(a)),this.viewerState.set(t,o),this.showActionError(e,p instanceof Error?p.message:"Action failed")}finally{this.pendingActions.delete(r)}}handleToggleReplyForm(e){if(this.openReplyFormUri===e){this.closeReplyForm();return}this.closeReplyForm();let t=this.container.querySelector(`.juttu-comment[data-uri="${CSS.escape(e)}"]`);if(!t)return;let n=document.createElement("div");n.className="juttu-reply-form";let r=document.createElement("textarea");r.className="juttu-reply-input",r.placeholder="Write a reply\u2026",r.rows=2;let{wrap:o,backdrop:s}=this.buildEditorWrap(r);n.appendChild(o),setTimeout(()=>this.syncBackdropStyles(s,r),0),r.addEventListener("input",()=>{this.updateBackdrop(s,r.value),this.handleMentionInput(r)}),r.addEventListener("scroll",()=>this.syncScroll(s,r)),r.addEventListener("keydown",p=>{p.key==="Escape"&&this.dismissAutocomplete(r)});let i=document.createElement("div");i.className="juttu-reply-form-actions";let a=document.createElement("span");a.className="juttu-reply-hint",a.textContent="Reply posts to Bluesky",i.appendChild(a);let l=document.createElement("button");l.className="juttu-reply-cancel",l.textContent="Cancel",i.appendChild(l);let c=document.createElement("button");c.className="juttu-reply-submit",c.textContent="Reply",c.disabled=!0,i.appendChild(c),n.appendChild(i);let d=t.querySelector(".juttu-comment-actions");d?.nextSibling?t.insertBefore(n,d.nextSibling):t.appendChild(n),this.openReplyFormUri=e,r.focus()}closeReplyForm(){if(!this.openReplyFormUri)return;this.container.querySelector(".juttu-reply-form")?.remove(),this.openReplyFormUri=null}async handlePost(){let e=this.container.querySelector(".juttu-compose-input"),t=this.container.querySelector(".juttu-submit-btn");if(!e||!t)return;let n=e.value.trim();if(!(!n||!this.rootPostUri||!this.rootPostCid)){t.disabled=!0,t.textContent="Posting\u2026",this.container.querySelector(".juttu-post-error")?.remove();try{let o=await(await fetch(`${this.config.apiUrl}/bsky/post`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({text:n,reply:{root:{uri:this.rootPostUri,cid:this.rootPostCid},parent:{uri:this.rootPostUri,cid:this.rootPostCid}}})}).then(this.checkApiResponse)).json();e.value="";let s=e.closest(".juttu-editor-wrap")?.querySelector(".juttu-editor-backdrop");if(s&&this.updateBackdrop(s,""),t.disabled=!0,t.textContent="Post comment",this.threadData&&this.currentUser){let i=this.makeSyntheticReply(o.uri,o.cid,n);this.threadData.replies||(this.threadData.replies=[]),this.threadData.replies.push(i),this.syncLocalCounts(this.threadData),this.renderWidget()}setTimeout(()=>this.refetchAndRender(),1500)}catch(r){t.disabled=!1,t.textContent="Post comment";let o=document.createElement("div");o.className="juttu-post-error",o.textContent=r instanceof Error?r.message:"Failed to post",t.insertAdjacentElement("afterend",o)}}}async handleSubmitReply(e){let t=this.container.querySelector(".juttu-reply-form");if(!t)return;let n=t.querySelector(".juttu-reply-input"),r=t.querySelector(".juttu-reply-submit");if(!n||!r)return;let o=n.value.trim();if(!o||!this.rootPostUri||!this.rootPostCid)return;let s=E(this.threadData,e);if(s){r.disabled=!0,r.textContent="Replying\u2026",t.querySelector(".juttu-reply-error")?.remove();try{let a=await(await fetch(`${this.config.apiUrl}/bsky/post`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({text:o,reply:{root:{uri:this.rootPostUri,cid:this.rootPostCid},parent:{uri:s.uri,cid:s.cid}}})}).then(this.checkApiResponse)).json();if(this.closeReplyForm(),this.threadData&&this.currentUser){let l=this.findThreadNode(this.threadData,e);if(l){let c=this.makeSyntheticReply(a.uri,a.cid,o);l.replies||(l.replies=[]),l.replies.push(c),this.syncLocalCounts(this.threadData),this.renderWidget()}}setTimeout(()=>this.refetchAndRender(),1500)}catch(i){r.disabled=!1,r.textContent="Reply";let a=document.createElement("div");a.className="juttu-reply-error",a.textContent=i instanceof Error?i.message:"Failed to post reply",t.appendChild(a)}}}makeSyntheticReply(e,t,n){let r=this.currentUser;return{$type:"app.bsky.feed.defs#threadViewPost",post:{uri:e,cid:t,author:{did:r.did,handle:r.handle,displayName:r.displayName,avatar:r.avatar},record:{text:n,createdAt:new Date().toISOString()},indexedAt:new Date().toISOString(),likeCount:0,repostCount:0,replyCount:0},replies:[]}}findThreadNode(e,t){if(e.post.uri===t)return e;for(let n of e.replies??[])if(n.$type==="app.bsky.feed.defs#threadViewPost"){let r=this.findThreadNode(n,t);if(r)return r}return null}async refetchAndRender(){if(this.rootPostUri)try{let e=await S(this.rootPostUri);this.threadData=e;let t=new Map;x(e,t);let n=new Map;for(let[o,s]of t){let i=this.viewerState.get(o);i&&(i.likeUri!==void 0||i.repostUri!==void 0)?n.set(o,{likeUri:i.likeUri==="pending"?s.likeUri:i.likeUri,repostUri:i.repostUri==="pending"?s.repostUri:i.repostUri}):n.set(o,s)}this.viewerState=n;let r=this.localCounts;this.syncLocalCounts(e);for(let[o,s]of this.viewerState){if(!s.likeUri&&!s.repostUri)continue;let i=r.get(o),a=this.localCounts.get(o);if(i&&a){let l=E(e,o)?.likeCount??0,c=E(e,o)?.repostCount??0;a.likes===l&&i.likes!==l&&(a.likes=i.likes),a.reposts===c&&i.reposts!==c&&(a.reposts=i.reposts)}}this.renderWidget()}catch{}}buildEditorWrap(e){let t=document.createElement("div");t.className="juttu-editor-wrap";let n=document.createElement("div");return n.className="juttu-editor-backdrop",n.setAttribute("aria-hidden","true"),t.appendChild(n),t.appendChild(e),{wrap:t,backdrop:n}}syncBackdropStyles(e,t){let n=window.getComputedStyle(t);for(let r of["paddingTop","paddingRight","paddingBottom","paddingLeft","borderTopWidth","borderRightWidth","borderBottomWidth","borderLeftWidth","fontFamily","fontSize","fontWeight","lineHeight","letterSpacing"])e.style[r]=n[r]}updateBackdrop(e,t){for(;e.firstChild;)e.removeChild(e.firstChild);for(let n of rt(t))if(n.type==="plain")e.appendChild(document.createTextNode(n.text));else{let r=document.createElement("span");r.className=`juttu-editor-highlight-${n.type}`,r.textContent=n.text,e.appendChild(r)}e.appendChild(document.createTextNode(`
`))}syncScroll(e,t){e.scrollTop=t.scrollTop}clearMentionState(){this.mentionDebounceTimer!==null&&(clearTimeout(this.mentionDebounceTimer),this.mentionDebounceTimer=null),this.activeSuggestionsTextarea=null}handleMentionInput(e){let t=e.selectionStart??e.value.length,r=e.value.slice(0,t).match(/@([a-zA-Z0-9][a-zA-Z0-9.-]*)$/);r?(this.activeSuggestionsTextarea=e,this.mentionDebounceTimer!==null&&clearTimeout(this.mentionDebounceTimer),this.mentionDebounceTimer=setTimeout(()=>{this.fetchMentionSuggestions(r[1],e)},300)):this.dismissAutocomplete(e)}async fetchMentionSuggestions(e,t){if(this.activeSuggestionsTextarea===t)try{let n=await fetch(`https://public.api.bsky.app/xrpc/app.bsky.actor.searchActorsTypeahead?q=${encodeURIComponent(e)}&limit=4`);if(!n.ok||this.activeSuggestionsTextarea!==t)return;let r=await n.json();this.renderAutocomplete(r.actors,t)}catch{this.dismissAutocomplete(t)}}renderAutocomplete(e,t){let n=t.closest(".juttu-editor-wrap");if(!n||(n.querySelector(".juttu-autocomplete")?.remove(),e.length===0))return;let r=document.createElement("ul");r.className="juttu-autocomplete";for(let o of e){let s=document.createElement("li"),i=document.createElement("button");if(i.className="juttu-autocomplete-item",i.type="button",o.avatar){let c=document.createElement("img");c.className="juttu-autocomplete-avatar",c.src=o.avatar,c.alt="",i.appendChild(c)}else{let c=document.createElement("div");c.className="juttu-autocomplete-avatar",c.style.background="var(--juttu-border-color)",i.appendChild(c)}let a=document.createElement("div");if(o.displayName){let c=document.createElement("div");c.style.fontWeight="600",c.textContent=o.displayName,a.appendChild(c)}let l=document.createElement("div");l.className="juttu-autocomplete-handle",l.textContent=`@${o.handle}`,a.appendChild(l),i.appendChild(a),i.addEventListener("mousedown",c=>{c.preventDefault(),this.applyMentionSuggestion(o.handle,t)}),s.appendChild(i),r.appendChild(s)}n.appendChild(r)}applyMentionSuggestion(e,t){let n=t.selectionStart??t.value.length,r=t.value.slice(0,n).replace(/@([a-zA-Z0-9][a-zA-Z0-9.-]*)$/,`@${e} `);t.value=r+t.value.slice(n);let o=r.length;setTimeout(()=>{t.focus(),t.setSelectionRange(o,o)},0),this.dismissAutocomplete(t);let s=t.closest(".juttu-editor-wrap")?.querySelector(".juttu-editor-backdrop");s&&this.updateBackdrop(s,t.value)}dismissAutocomplete(e){this.mentionDebounceTimer!==null&&(clearTimeout(this.mentionDebounceTimer),this.mentionDebounceTimer=null),this.activeSuggestionsTextarea=null,e.closest(".juttu-editor-wrap")?.querySelector(".juttu-autocomplete")?.remove()}showActionError(e,t){e.parentElement?.querySelector(".juttu-action-error")?.remove();let n=document.createElement("div");n.className="juttu-action-error",n.textContent=t,e.insertAdjacentElement("afterend",n),setTimeout(()=>n.remove(),4e3)}renderLinkingUI(){this.container.innerHTML="";let e=this.makeRoot(),t=document.createElement("div");switch(t.className="juttu-linking",this.linkingStep){case"setup":t.appendChild(this.makeLinkingSetup());break;case"login":t.appendChild(this.makeLinkingLoginForm());break;case"metadata":t.appendChild(this.makeLinkingMetadata());break;case"choose-method":t.appendChild(this.makeLinkingChooseMethod());break;case"write-post":t.appendChild(this.makeLinkingWritePost());break;case"select-post":t.appendChild(this.makeLinkingSelectPost());break}e.appendChild(t),e.addEventListener("click",o=>this.handleLinkingClick(o));let n=document.createElement("div");n.className="juttu-footer";let r=document.createElement("a");r.className="juttu-powered-by",r.href="https://juttu.app",r.target="_blank",r.rel="noopener noreferrer",r.textContent="Powered by Juttu",n.appendChild(r),e.appendChild(n),this.container.appendChild(e)}makeLinkingSetup(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title";let n=document.createElement("p");n.className="juttu-linking-desc",this.documentRecord?(t.textContent="Comments not linked yet",n.textContent="Link this article to a Bluesky post to enable the comment thread."):(t.textContent="Comments not set up yet",n.textContent="Set up a Bluesky-powered comment thread for this article."),e.appendChild(t),e.appendChild(n);let r=document.createElement("button");return r.className="juttu-linking-start-btn",r.textContent=this.documentRecord?"Link comments":"Set up comments",e.appendChild(r),e}makeLinkingLoginForm(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Sign in as the document owner",e.appendChild(t);let n=document.createElement("button");return n.className="juttu-linking-login-btn",n.textContent="Login with Bluesky \u2192",e.appendChild(n),e}makeLinkingMetadata(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Article details",e.appendChild(t);let n=document.createElement("div");n.className="juttu-linking-field";let r=document.createElement("label");r.className="juttu-linking-label",r.textContent="Title *",n.appendChild(r);let o=document.createElement("input");o.type="text",o.className="juttu-linking-input juttu-linking-title-input",o.placeholder="Article title",o.value=this.linkingTitle,n.appendChild(o),e.appendChild(n);let s=document.createElement("div");s.className="juttu-linking-field";let i=document.createElement("label");i.className="juttu-linking-label",i.textContent="Description (optional)",s.appendChild(i);let a=document.createElement("input");a.type="text",a.className="juttu-linking-input juttu-linking-desc-input",a.placeholder="Short description",a.value=this.linkingDescription,s.appendChild(a),e.appendChild(s);let l=document.createElement("button");return l.className="juttu-linking-continue-btn",l.textContent="Continue",l.disabled=!this.linkingTitle.trim(),e.appendChild(l),o.addEventListener("input",()=>{l.disabled=!o.value.trim()}),e}makeLinkingChooseMethod(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Link a Bluesky post",e.appendChild(t);let n=document.createElement("p");n.className="juttu-linking-desc",n.textContent="This post becomes the root of the comment thread.",e.appendChild(n);let r=document.createElement("div");r.className="juttu-linking-methods";let o=document.createElement("button");o.className="juttu-linking-method-btn",o.dataset.method="write";let s=document.createElement("div");s.className="juttu-linking-method-title",s.textContent="Write a new post";let i=document.createElement("div");i.className="juttu-linking-method-desc",i.textContent="Compose a post to announce this article",o.appendChild(s),o.appendChild(i),r.appendChild(o);let a=document.createElement("button");a.className="juttu-linking-method-btn",a.dataset.method="select";let l=document.createElement("div");l.className="juttu-linking-method-title",l.textContent="Use an existing post";let c=document.createElement("div");return c.className="juttu-linking-method-desc",c.textContent="Pick from your recent Bluesky posts",a.appendChild(l),a.appendChild(c),r.appendChild(a),e.appendChild(r),e}makeLinkingWritePost(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Write a post",e.appendChild(t);let n=document.createElement("textarea");n.className="juttu-linking-textarea",n.placeholder="Share this article on Bluesky\u2026",e.appendChild(n);let r=document.createElement("div");r.style.cssText="display:flex;gap:0.5rem;margin-top:0.5rem;";let o=document.createElement("button");o.className="juttu-linking-back-btn",o.style.cssText="background:none;border:1px solid var(--juttu-border-color);border-radius:var(--juttu-radius);padding:0.4rem 0.75rem;cursor:pointer;font-size:1rem;color:var(--juttu-text-muted);font-family:var(--juttu-font-family);",o.textContent="\u2190 Back",r.appendChild(o);let s=document.createElement("button");return s.className="juttu-linking-continue-btn juttu-linking-write-submit",s.textContent="Post & Link",s.disabled=!0,r.appendChild(s),e.appendChild(r),n.addEventListener("input",()=>{s.disabled=!n.value.trim()}),e}makeLinkingSelectPost(){let e=document.createElement("div"),t=document.createElement("p");t.className="juttu-linking-title",t.textContent="Select a post",e.appendChild(t);let n=document.createElement("button");n.className="juttu-linking-back-btn",n.style.cssText="background:none;border:1px solid var(--juttu-border-color);border-radius:var(--juttu-radius);padding:0.3rem 0.65rem;cursor:pointer;font-size:0.875rem;color:var(--juttu-text-muted);font-family:var(--juttu-font-family);margin-bottom:0.75rem;",n.textContent="\u2190 Back",e.appendChild(n);let r=document.createElement("div");if(r.className="juttu-linking-post-list",this.userPosts.length===0){let o=document.createElement("p");o.className="juttu-linking-spinner",o.textContent="Loading posts\u2026",r.appendChild(o)}else for(let o of this.userPosts){let s=document.createElement("button");s.className="juttu-linking-post-item",s.dataset.uri=o.uri,s.dataset.cid=o.cid;let i=document.createElement("div");i.className="juttu-linking-post-text",i.textContent=o.text.length>180?o.text.slice(0,180)+"\u2026":o.text;let a=document.createElement("div");a.className="juttu-linking-post-date",a.textContent=$(o.createdAt),s.appendChild(i),s.appendChild(a),r.appendChild(s)}return e.appendChild(r),e}handleLinkingClick(e){let t=e.target;if(t.closest(".juttu-linking-start-btn")){this.resolveAuth();return}if(t.closest(".juttu-linking-login-btn")){this.openLoginPopup();return}if(t.closest(".juttu-linking-continue-btn")&&!t.closest(".juttu-linking-write-submit")){let o=this.container.querySelector(".juttu-linking-title-input"),s=this.container.querySelector(".juttu-linking-desc-input"),i=o?.value.trim()??"";if(!i)return;this.linkingTitle=i,this.linkingDescription=s?.value.trim()??"",this.linkingStep="choose-method",this.renderLinkingUI();return}let n=t.closest(".juttu-linking-method-btn");if(n){n.dataset.method==="write"?(this.linkingStep="write-post",this.renderLinkingUI()):n.dataset.method==="select"&&(this.linkingStep="select-post",this.userPosts=[],this.renderLinkingUI(),this.fetchUserPostsAndRender());return}if(t.closest(".juttu-linking-write-submit")){let s=this.container.querySelector(".juttu-linking-textarea")?.value.trim()??"";if(!s)return;this.handleLinkingCreatePost(s);return}let r=t.closest(".juttu-linking-post-item");if(r){let o=r.dataset.uri,s=r.dataset.cid;o&&s&&this.callPutDocument(o,s);return}if(t.closest(".juttu-linking-back-btn")){this.linkingStep="choose-method",this.renderLinkingUI();return}}async fetchUserPostsAndRender(){if(this.currentUser)try{let e=await fetch(`https://public.api.bsky.app/xrpc/app.bsky.feed.getAuthorFeed?actor=${encodeURIComponent(this.currentUser.handle)}&filter=posts_no_replies&limit=20`);if(!e.ok)throw new Error(`Failed to fetch posts: ${e.status}`);let t=await e.json();this.userPosts=t.feed.filter(n=>!n.reason).map(n=>({uri:n.post.uri,cid:n.post.cid,text:n.post.record.text,createdAt:n.post.record.createdAt})),this.linkingStep==="select-post"&&this.renderLinkingUI()}catch(e){if(this.linkingStep==="select-post"){let t=this.container.querySelector(".juttu-linking-post-list");if(t){t.innerHTML="";let n=document.createElement("p");n.className="juttu-linking-error",n.textContent=e instanceof Error?e.message:"Failed to load posts",t.appendChild(n)}}}}async handleLinkingCreatePost(e){let t=this.container.querySelector(".juttu-linking-write-submit");t&&(t.disabled=!0,t.textContent="Posting\u2026");try{let r=await(await fetch(`${this.config.apiUrl}/bsky/post`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({text:e})}).then(this.checkApiResponse)).json();await this.callPutDocument(r.uri,r.cid)}catch(n){t&&(t.disabled=!1,t.textContent="Post & Link");let r=document.createElement("p");r.className="juttu-linking-error",r.textContent=n instanceof Error?n.message:"Failed to post",this.container.querySelector(".juttu-linking")?.appendChild(r)}}async callPutDocument(e,t){if(!this.documentAtUri)return;let n=this.container.querySelector(".juttu-linking");if(n){let i=document.createElement("p");i.className="juttu-linking-spinner",i.textContent="Linking\u2026",n.appendChild(i)}let r={uri:e,cid:t},o=new Date().toISOString(),s;this.documentRecord?s={...this.documentRecord,bskyPostRef:r,updatedAt:o}:s={$type:"site.standard.document",site:window.location.origin,title:this.linkingTitle,description:this.linkingDescription||void 0,path:window.location.pathname||void 0,publishedAt:o,updatedAt:o,bskyPostRef:r};try{await fetch(`${this.config.apiUrl}/atproto/document`,{method:"PUT",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({rkey:this.documentAtUri.rkey,record:s})}).then(this.checkApiResponse),this.documentAtUri=null,this.documentRecord=null,this.rootPostUri=e,this.rootPostCid=t;let i=await S(e);this.threadData=i,x(i,this.viewerState),this.renderWidget()}catch(i){let a=this.container.querySelector(".juttu-linking");a?.querySelector(".juttu-linking-spinner")?.remove();let l=document.createElement("p");l.className="juttu-linking-error",l.textContent=i instanceof Error?i.message:"Failed to link document",a?.appendChild(l)}}destroy(){this.loginPollInterval!==null&&clearInterval(this.loginPollInterval);try{this.loginPopup?.close()}catch{}this.container.innerHTML=""}};if(typeof window<"u"){let e=function(t){let n=t.getAttribute("data-api-url")??"https://api.juttu.app",r=t.getAttribute("data-theme")??"auto",o=document.getElementById("juttu-comments");if(!o){console.error('Juttu: No element with id="juttu-comments" found on this page');return}new T(o,{apiUrl:n,theme:r})};dt=e,window.JuttuWidget=T;let u=document.currentScript;u&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>e(u)):e(u))}var dt;})();
