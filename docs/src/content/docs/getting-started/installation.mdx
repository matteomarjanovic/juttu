...
```sh
npx juttu site-document YOUR_HANDLE
```
...
That resolves your handle to its DID, then prints a complete `<link rel="site.standard.document" ...>` tag with a valid TID. Run it once per article and keep the generated TID stable.
